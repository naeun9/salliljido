...
// 않았다. 힐링·레저에서 3을 준 이유: 그 두 테마는 "무엇을 보느냐"보다
// "몸을 어떻게 쓰느냐"가 중심이라 걷는 길이 오후를 채우기에 가장 알맞다.
// 문화탐방·워케이션에서는 1로 두어 있으면 가끔 나오는 정도로만 섞인다.
const AFTERNOON_WEIGHTS = {
  힐링: { 자연: 3, 문화: 2, 체험: 2, 역사: 1, 레저스포츠: 1, 걷기코스: 3 },
  레저: { 레저스포츠: 3, 체험: 3, 자연: 2, 역사: 1, 문화: 1, 걷기코스: 3 },
...
function buildAfternoonPool(listings, theme) {
  ...
  if (!entry.afternoon[key]) {
    ...
    entry.afternoon[key] = weightedMerge([
      ...
      { items: listings["걷기 코스"] || [], weight: w.걷기코스 },
    ]);
  }
