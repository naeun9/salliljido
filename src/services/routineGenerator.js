...
// 걷기코스는 기존 분류에 얹기만 한다 — 다른 분류의 가중치는 하나도 건드리지
// 않았다. 처음에는 힐링·레저에 3을 줬는데 14일 계획을 실측해 보니 코스가
// 4일에 한 번꼴로 들어갔다(힐링 4/14일). 두루누비 코스는 파일럿 지역 기준
// 최단이 2시간 30분, 중앙값이 4시간 30분이라 들어가는 날은 오후가 통째로
// 사라진다. 며칠~한 달 머무는 서비스에서 그 빈도는 과하다.
//
// 그래서 가중치는 1로 낮추고, 실제 제한은 아래 COURSE_LIMIT(풀에 넣는 개수)
// 으로 건다. 가중치는 "얼마나 자주"가 아니라 "언제쯤" 나오는지만 정한다.
const AFTERNOON_WEIGHTS = {
  힐링: { 자연: 3, 문화: 2, 체험: 2, 역사: 1, 레저스포츠: 1, 걷기코스: 1 },
  레저: { 레저스포츠: 3, 체험: 3, 자연: 2, 역사: 1, 문화: 1, 걷기코스: 1 },
  문화탐방: { 역사: 3, 문화: 3, 체험: 2, 자연: 1, 레저스포츠: 1, 걷기코스: 1 },
  워케이션: { 자연: 3, 문화: 2, 역사: 2, 체험: 1, 레저스포츠: 1, 걷기코스: 1 },
};

// 한 계획에 넣을 걷기 코스의 최대 개수. 자동 생성은 후보를 한 번 쓰면
// 다시 쓰지 않으므로, 풀에 넣는 개수를 자르면 그대로 배치 횟수의 상한이
// 된다 — 체류가 7일이든 30일이든 이 수를 넘지 않는다.
// 걷는 게 중심인 힐링·레저는 2회, 나머지 테마는 1회까지만 둔다.
const COURSE_LIMIT = { 힐링: 2, 레저: 2, 문화탐방: 1, 워케이션: 1 };
...
// 자동 생성에 넣을 걷기 코스를 짧은 것부터 limit개만 고른다.
function shortestCourses(listings, limit) {
  return [...(listings["걷기 코스"] || [])]
    .sort((a, b) => (a.courseMinutes || 0) - (b.courseMinutes || 0))
...
function buildAfternoonPool(listings, theme) {
  ...
  if (!entry.afternoon[key]) {
    ...
    entry.afternoon[key] = weightedMerge([
      ...
      // 걷기 코스는 오후 슬롯에만 넣는다. 두루누비 코스는 짧은 것도 2시간이
      // 넘어(최단 150분) 오전에 넣으면 점심·저녁이 밀린다.
      // 짧은 것부터 정렬해 넣는다 — 자동 생성이 고르는 코스가 5시간짜리
      // 종주 구간이 되지 않게 한다.
      { items: shortestCourses(listings, COURSE_LIMIT[key] ?? 1), weight: w.걷기코스 },
    ]);
  }
